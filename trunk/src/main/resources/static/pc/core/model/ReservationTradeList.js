...
//定义ViewModel对象
var ReservationTradeViewModel = function () {  
	self=this;
    //添加动态监视数组对象
    self.reservationTradeList = ko.observableArray([]);
    
    //重载数据
    self.reloadData = function(){
    	myAjax("/reservationtrades", "GET", query, function (data){
			var mappedTasks = $.map(data.data, function(item) { return new Node(item) });  
			self.reservationTradeList(mappedTasks);
			myPage = data.page;
			bindPage();
			    
			$("table tbody td .tomodify").bind(function(){
				ChangeUrl('./core/ReservationTrade.html?action=Edit&id='+$(this).attr('data'));
			});
		}, true);
    };
	...
	self.search = function(obj) {
		...
		myAjax("/reservationtrades", "GET", query, doQueryActionSuccess, true);
    };
    
    //新增
    self.add = function(obj) {
    	ChangeUrl('./core/ReservationTrade.html?action=Add');
    };
    
    //修改
    self.modify=function(obj){
    	ChangeUrl('./core/ReservationTrade.html?action=Edit&id='+obj.detailId());
    };
    ...
    self.delete=function(obj){
    	parent.dialog({
            ...
            ok: function () {
		    	...
		    	myAjax("/reservationtrade/"+id, "DELETE", null, reloadDate, false);
			}
        }).showModal();
    }
	    ...
	    parent.dialog({
	        ...
	        ok: function () {
	        	$(".checkall input:checked").each(function(i){
	        		myAjax("/reservationtrade/"+id, "DELETE", null, null, false);
	        	});
	        	location.reload();
	        },
	        cancelValue: '取消',
...
$().ready(function(){
	viewMode = new ReservationTradeViewModel();
...
//            if (type != 'init') {
//            	ChangeUrl('./core/ReservationTradeList.html?page=' + num);
//            }
        }
    });
}
