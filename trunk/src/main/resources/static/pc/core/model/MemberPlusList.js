...
function Node(obj) {
	...
	this.plusName = ko.observable(obj.plusName); 
...
//定义ViewModel对象
var MemberPlusViewModel = function () {  
	self=this;
    //添加动态监视数组对象
    self.memberPlusList = ko.observableArray([]);
    
    //重载数据
    self.reloadData = function(){
    	myAjax("/memberpluss", "GET", query, function (data){
			var mappedTasks = $.map(data.data, function(item) { return new Node(item) });  
			self.memberPlusList(mappedTasks);
			myPage = data.page;
			bindPage();
			    
			$("table tbody td .tomodify").bind(function(){
				ChangeUrl('./core/MemberPlus.html?action=Edit&id='+$(this).attr('data'));
			});
		}, true);
    };
	...
	self.search = function(obj) {
		...
		myAjax("/memberpluss", "GET", query, doQueryActionSuccess, true);
    };
    
    //新增
    self.add = function(obj) {
    	ChangeUrl('./core/MemberPlus.html?action=Add');
    };
    
    //修改
    self.modify=function(obj){
    	ChangeUrl('./core/MemberPlus.html?action=Edit&id='+obj.plusId());
    };
    ...
    self.delete=function(obj){
    	parent.dialog({
            ...
            ok: function () {
		    	...
		    	myAjax("/memberplus/"+id, "DELETE", null, reloadDate, false);
			}
        }).showModal();
    }
	    ...
	    parent.dialog({
	        ...
	        ok: function () {
	        	$(".checkall input:checked").each(function(i){
	        		myAjax("/memberplus/"+id, "DELETE", null, null, false);
	        	});
	        	location.reload();
	        },
	        cancelValue: '取消',
...
$().ready(function(){
	viewMode = new MemberPlusViewModel();
...
//            if (type != 'init') {
//            	ChangeUrl('./core/MemberPlusList.html?page=' + num);
//            }
        }
    });
}
