...
function Node(obj) {
	...
	this.saleStatus = ko.observable(obj.saleStatus); 
	this.note = ko.observable(obj.note); 
	this.goodsImg = ko.observable(obj.goodsImg); 
	this.isShow = ko.observable(obj.isShow); 
	this.canCancel = ko.observable(obj.canCancel); 
...
var GoodsViewModel = function () {  
    ...
    self.reloadData = function(){
    	myAjax("/goodss", "GET", query, function (data){
			var mappedTasks = $.map(data.data, function(item) { return new Node(item) });  
			self.goodsList(mappedTasks);
			myPage = data.page;
			bindPage();
			    
			$("table tbody td .tomodify").bind(function(){
				ChangeUrl('./order/Goods.html?action=Edit&id='+$(this).attr('data'));
			});
		}, true);
    };
	...
	self.search = function(obj) {
		...
		myAjax("/goodss", "GET", query, doQueryActionSuccess, true);
    };
    
    //新增
    self.add = function(obj) {
    	ChangeUrl('./order/Goods.html?action=Add');
    };
    
    //修改
    self.modify=function(obj){
    	ChangeUrl('./order/Goods.html?action=Edit&id='+obj.goodsId());
    };
    ...
    self.delete=function(obj){
    	parent.dialog({
            ...
            ok: function () {
		    	...
		    	myAjax("/goods/"+id, "DELETE", null, reloadDate, false);
			}
        }).showModal();
    }
	    ...
	    parent.dialog({
	        ...
	        ok: function () {
	        	$(".checkall input:checked").each(function(i){
	        		myAjax("/goods/"+id, "DELETE", null, null, false);
	        	});
	        	location.reload();
	        },
	        cancelValue: '取消',
...
//            if (type != 'init') {
//            	ChangeUrl('./order/GoodsList.html?page=' + num);
//            }
        }
    });
}
