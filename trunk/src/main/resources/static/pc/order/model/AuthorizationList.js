...
function Node(obj) {
	...
	this.deviceId = ko.observable(obj.deviceId); 
...
var AuthorizationViewModel = function () {  
	self=this;
    //添加动态监视数组对象
    self.authorizationList = ko.observableArray([]);
    
    //重载数据
    self.reloadData = function(){
    	myAjax("/authorizations", "GET", query, function (data){
			var mappedTasks = $.map(data.data, function(item) { return new Node(item) });  
			self.authorizationList(mappedTasks);
			myPage = data.page;
			bindPage();
			    
			$("table tbody td .tomodify").bind(function(){
				ChangeUrl('./order/Authorization.html?action=Edit&id='+$(this).attr('data'));
			});
		}, true);
    };
	...
	self.search = function(obj) {
		...
		myAjax("/authorizations", "GET", query, doQueryActionSuccess, true);
    };
    
    //新增
    self.add = function(obj) {
    	ChangeUrl('./order/Authorization.html?action=Add');
    };
    
    //修改
    self.modify=function(obj){
    	ChangeUrl('./order/Authorization.html?action=Edit&id='+obj.authorizationId());
    };
    ...
    self.delete=function(obj){
    	parent.dialog({
            ...
            ok: function () {
		    	...
		    	myAjax("/authorization/"+id, "DELETE", null, reloadDate, false);
			}
        }).showModal();
    }
	    ...
	    parent.dialog({
	        ...
	        ok: function () {
	        	$(".checkall input:checked").each(function(i){
	        		myAjax("/authorization/"+id, "DELETE", null, null, false);
	        	});
	        	location.reload();
	        },
	        cancelValue: '取消',
...
$().ready(function(){
	viewMode = new AuthorizationViewModel();
...
//            if (type != 'init') {
//            	ChangeUrl('./order/AuthorizationList.html?page=' + num);
//            }
        }
    });
}
