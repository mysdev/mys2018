...
function Node(obj) {
	...
	this.packageName = ko.observable(obj.packageName); 
	this.pinyin = ko.observable(obj.pinyin); 
	this.price = ko.observable(obj.price); 
	this.validDate = ko.observable(obj.validDate); 
...
var PackagesViewModel = function () {  
	self=this;
    //添加动态监视数组对象
    self.packagesList = ko.observableArray([]);
    
    //重载数据
    self.reloadData = function(){
    	myAjax("/packagess", "GET", query, function (data){
			var mappedTasks = $.map(data.data, function(item) { return new Node(item) });  
			self.packagesList(mappedTasks);
			myPage = data.page;
			bindPage();
			    
			$("table tbody td .tomodify").bind(function(){
				ChangeUrl('./order/Packages.html?action=Edit&id='+$(this).attr('data'));
			});
		}, true);
    };
	...
	self.search = function(obj) {
		...
		myAjax("/packagess", "GET", query, doQueryActionSuccess, true);
    };
    
    //新增
    self.add = function(obj) {
    	ChangeUrl('./order/Packages.html?action=Add');
    };
    
    //修改
    self.modify=function(obj){
    	ChangeUrl('./order/Packages.html?action=Edit&id='+obj.packageId());
    };
    ...
    self.delete=function(obj){
    	parent.dialog({
            ...
            ok: function () {
		    	...
		    	myAjax("/packages/"+id, "DELETE", null, reloadDate, false);
			}
        }).showModal();
    }
	    ...
	    parent.dialog({
	        ...
	        ok: function () {
	        	$(".checkall input:checked").each(function(i){
	        		myAjax("/packages/"+id, "DELETE", null, null, false);
	        	});
	        	location.reload();
	        },
	        cancelValue: '取消',
...
$().ready(function(){
	viewMode = new PackagesViewModel();
...
//            if (type != 'init') {
//            	ChangeUrl('./order/PackagesList.html?page=' + num);
//            }
        }
    });
}
