...
function getOptionStr(group){
	...
	for(var i=0 ;i<dictionaryObj.length;i++){
		if(dictionaryObj[i].code==group){
			var array = dictionaryObj[i].dictionarys;
			for(var j=0;j<array.length;j++){
				if('health'==group || 'economy'==group || 'personnelProperty'==group || 'marriageStatus'==group || 'politicalStatus'==group || 'building_self_type'==group || 'building_type'==group || 'building_use' ==group || 'building_structure'==group ){
					rs +="<li><table><tr><td><input type='checkbox' id='con"+group+"_"+array[j].code+"' onclick='add_contions(\""+group+"\", \""+array[j].code+"\", \""+array[j].value+"\")'/></td><td style='font-size:12px;padding-top:4px;'>"+array[j].value+"</td></tr></table></li> ";
				}else if('nation' == group){
					ss +="<div class='div_f div_nationw div_mt_t_t'><table><tr><td><input type='checkbox' id='con"+group+"_"+array[j].code+"' onclick='add_contions(\""+group+"\", \""+array[j].code+"\", \""+array[j].value+"\")'/></td><td style='font-size:12px;padding-top:4px;'>"+array[j].value+"</td></tr></table></div> ";
				}else if('education'==group){
					ss +="<div class='div_f div_educationW div_mt_t_t'><table><tr><td><input type='checkbox' id='con"+group+"_"+array[j].code+"' onclick='add_contions(\""+group+"\", \""+array[j].code+"\", \""+array[j].value+"\")'/></td><td style='font-size:12px;padding-top:4px;'>"+array[j].value+"</td></tr></table></div> ";
				}else if('population_business'==group){
					ss +="<div class='div_f div_popw div_mt_t_t'><table><tr><td><input type='checkbox' id='con"+group+"_"+array[j].code+"' onclick='add_contions(\""+group+"\", \""+array[j].code+"\", \""+array[j].value+"\")'/></td><td style='font-size:12px;padding-top:4px;'>"+array[j].value+"</td></tr></table></div> ";
				}else{
					rs +="<li><a href='javascript:;' selectid='"+array[j].code+"' onclick='getContion(\""+group+"\", \""+array[j].code+"\", \""+array[j].value+"\")'>"+array[j].value+"</a></li> ";
				}
			}
		}
	}
