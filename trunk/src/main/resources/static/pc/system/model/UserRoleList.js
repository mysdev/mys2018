...
var UserRoleViewModel = function () {  
    ...
    self.reloadData = function(){
    	myAjax("/userroles", "GET", query, function (data){
			var mappedTasks = $.map(data.data, function(item) { return new Node(item) });  
			self.userRoleList(mappedTasks);
			myPage = data.page;
			bindPage();
			    
			$("table tbody td .tomodify").bind(function(){
				ChangeUrl('./system/UserRole.html?action=Edit&id='+$(this).attr('data'));
			});
		}, true);
    };
	...
	self.search = function(obj) {
		...
		myAjax("/userroles", "GET", query, doQueryActionSuccess, true);
    };
    
    //新增
    self.add = function(obj) {
    	ChangeUrl('./system/UserRole.html?action=Add');
    };
    
    //修改
    self.modify=function(obj){
    	ChangeUrl('./system/UserRole.html?action=Edit&id='+obj.userId());
    };
    ...
    self.delete=function(obj){
    	parent.dialog({
            ...
            ok: function () {
		    	...
		    	myAjax("/userrole/"+id, "DELETE", null, reloadDate, false);
			}
        }).showModal();
    }
	    ...
	    parent.dialog({
	        ...
	        ok: function () {
	        	$(".checkall input:checked").each(function(i){
	        		myAjax("/userrole/"+id, "DELETE", null, null, false);
	        	});
	        	location.reload();
	        },
	        cancelValue: '取消',
...
$().ready(function(){
	viewMode = new UserRoleViewModel();
...
//            if (type != 'init') {
//            	ChangeUrl('./system/UserRoleList.html?page=' + num);
//            }
        }
    });
}
