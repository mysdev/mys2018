...
function Node(obj) {
	...
	this.parentCode = ko.observable(obj.parentCode); 
...
//定义ViewModel对象
var DictionaryViewModel = function () {  
	self=this;
    //添加动态监视数组对象
    ...
    self.reloadData = function(){
    	myAjax("/dictionarys", "GET", query, function (data){
			var mappedTasks = $.map(data.data, function(item) { return new Node(item) });  
			self.dictionaryList(mappedTasks);
			myPage = data.page;
			bindPage();
			    
			$("table tbody td .tomodify").bind(function(){
				ChangeUrl('./system/Dictionary.html?action=Edit&id='+$(this).attr('data'));
			});
		}, true);
    };
	...
	self.search = function(obj) {
		...
		myAjax("/dictionarys", "GET", query, doQueryActionSuccess, true);
    };
    
    //新增
    self.add = function(obj) {
    	ChangeUrl('./system/Dictionary.html?action=Add');
    };
    
    //修改
    self.modify=function(obj){
    	ChangeUrl('./system/Dictionary.html?action=Edit&id='+obj.id());
    };
    ...
    self.delete=function(obj){
    	parent.dialog({
            ...
            ok: function () {
		    	...
		    	myAjax("/dictionary/"+id, "DELETE", null, reloadDate, false);
			}
        }).showModal();
    }
	    ...
	    parent.dialog({
	        ...
	        ok: function () {
	        	$(".checkall input:checked").each(function(i){
	        		myAjax("/dictionary/"+id, "DELETE", null, null, false);
	        	});
	        	location.reload();
	        },
	        cancelValue: '取消',
...
$().ready(function(){
	viewMode = new DictionaryViewModel();
...
//            if (type != 'init') {
//            	ChangeUrl('./system/DictionaryList.html?page=' + num);
//            }
        }
    });
}
