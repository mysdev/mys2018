var detailview = $.extend({}, $.fn.datagrid.defaults.view, {
	addExpandColumn : function(target, index) {
		...
		function _add(rowIndex) {
			...
			cc.push('<div class="datagrid-row-expander datagrid-row-expand" row-index=' + rowIndex
					+ ' style="display: inline-block; width: 16px; height: 16px; margin: 0;  cursor: pointer;" />');
			cc.push('</div>');
			cc.push('</td>');
			if (tr.is(':empty')) {
				tr.html(cc.join(''));
	...
	onAfterRender : function(target) {
		...
		this.canUpdateDetail = true; // define if to update the detail
										// content when 'updateRow' method is
										// called;
